...
export const getInputClass = (hasError: boolean) =>
  `box-border flex gap-2 rounded-lg border px-4 py-[0.6875rem] text-[0.875rem] md:text-[1rem]  placeholder:text-gray40 focus-within:border-violet-500 outline-0 dark:bg-black80 ${
...
export function InputWithImageWrapper({
  ...
  return (
    <div
      className={`placeholder:text-gray40 dark:bg-black80 box-border flex gap-2 rounded-lg border bg-white px-4 py-[0.6875rem] focus-within:border-violet-500 ${
        hasError ? 'border-red' : 'border-gray30'
      }`}
    >
      {children}
...
export function ErrorMessage({ message }: { message: string }) {
  return message ? <p className='text-red text-[0.875rem]'>{message}</p> : null
}
...
export function Label({ label, isRequired, htmlFor }: LabelProps) {
  return (
    ...
    >
      {label}
      {isRequired && <p className='inline text-violet-500'> *</p>}
    </label>
  )
}
